...
func (e *ServiceSyncer) syncServiceAndEndpoints(ctx context.Context, fromService *corev1.Service, to types.NamespacedName) (ctrl.Result, error) {
	...
	if err != nil {
		...
		e.Log.Infof("NC: From Service %s/%s; ClusterIP: %s ", fromService.GetNamespace(), fromService.GetName(), fromService.Spec.ClusterIP)
		if fromService.Spec.ClusterIP == corev1.ClusterIPNone {
			...
			e.Log.Infof("NC: found endpoints %s/%s ", fromService.GetNamespace(), fromService.GetName())
